...
export default function ReceiptScreen() {
  ...
  const loadReceiptData = async () => {
    try {
      setLoading(true);
      if (receiptId) {
        // Load from recentReceipts
        const stored = await AsyncStorage.getItem('recentReceipts');
        if (stored) {
          const receipts = JSON.parse(stored);
          const found = receipts.find((r: any) => r.id === receiptId);
          if (found) {
            setReceiptItems(found.items);
            setOriginalItems(found.items);
            setIsModified(false);
            setLoading(false);
            return;
          }
  ...
  const updateRecentReceipt = async (updatedItems: ReceiptItem[]) => {
    if (!receiptId) return;
    try {
      const stored = await AsyncStorage.getItem('recentReceipts');
      if (stored) {
        ...
        if (idx !== -1) {
          ...
          receipts[idx].items = updatedItems;
          receipts[idx].total = total;
          await AsyncStorage.setItem('recentReceipts', JSON.stringify(receipts));
        }
      }
    } catch (e) {
      console.error('Error updating recent receipt:', e);
    }
  };

  const saveReceiptData = async (updatedItems?: ReceiptItem[]) => {
    try {
      const itemsToSave = updatedItems || receiptItems;
      // Save modified data separately
      await AsyncStorage.setItem('modifiedReceiptData', JSON.stringify(itemsToSave));
      setIsModified(true);
      if (receiptId) {
        updateRecentReceipt(itemsToSave);
      }
    } catch (error) {
  ...
  const deleteItem = async (index: number) => {
    ...
    setReceiptItems(newItems);
    await saveReceiptData(newItems);
  };
        ...
        {loading ? (
          ...
          <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent}>
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                const newItems = [...receiptItems, { name: '', price: '', quantity: 1 }];
                setReceiptItems(newItems);
                saveReceiptData(newItems);
              }}
            >
